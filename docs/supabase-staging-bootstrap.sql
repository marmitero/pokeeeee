-- Bootstrap auditável do Supabase STAGING — gerado das migrations Drizzle.
-- Execute somente em um projeto staging novo, com o schema public vazio.
-- Uma falha reverte tudo por causa da transação.
  ...
  IF EXISTS (
    ...
    WHERE table_schema = 'public'
      AND table_name IN ('users', 'sessions', 'user_pokemon', 'game_maps',
        'shop_items', 'gym_leaders', 'user_badges', 'pvp_battles',
        'battles', 'rate_limits', 'chat_messages')
  ) THEN
    RAISE EXCEPTION 'Bootstrap recusado: já existem tabelas do jogo em public';
  END IF;
END $$;

-- ================================================================
-- 0000_military_kitty_pryde.sql
-- sha256: f9c17bcfd691716a98b2581cd77c5eac457579e5a56376a7ad8589beea080c75
-- ================================================================
...
-- ================================================================
-- 0001_useful_vin_gonzales.sql
-- sha256: 9c6b89334dfb4b0a46835ef16f50a129842c45e8278f3f63c9592996b12ee683
-- ================================================================
...
-- ================================================================
-- 0002_long_squadron_sinister.sql
-- sha256: 73220ce5753196cdc206f5dc31159588997a9c4dcbf23a7eb5ee99ee4c698539
-- ================================================================
...
-- ================================================================
-- 0003_empty_gambit.sql
-- sha256: ca83039e23329d05acc09f881222623c420750c7d019e157e03668f863dd4654
-- ================================================================
...
-- Bloqueia a Data API: sem policies, anon/authenticated não enxergam linhas.
...
-- Alinha o histórico para que drizzle-kit não reaplique o bootstrap.
...
-- Resultado de verificação (deve retornar 11 e 4):
SELECT
  (SELECT count(*) FROM information_schema.tables
   WHERE table_schema = 'public'
     AND table_name IN ('users', 'sessions', 'user_pokemon', 'game_maps',
       'shop_items', 'gym_leaders', 'user_badges', 'pvp_battles',
       'battles', 'rate_limits', 'chat_messages')) AS game_tables,
